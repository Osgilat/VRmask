    ...
    void Update()
    {
        if(!IsConfigured || !blendShapesEnabled){
            Debug.Log(DateTime.Now.ToString("M/d/yyyy") + " "
                ...
                + System.DateTime.Now.Millisecond + "," + "Not configured or blendshapes are not enabled");
            //loggingSystem.writeMessageWithTimestampToLog("Not configured or blendshapes are not enabled");
            return;
        }
        ...
        foreach (var configuration in ExpresionConfigurations)
        {
            foreach (var range in configuration.BlendShapeRanges)
            {
                ...
                if(currentBlendShapes.ContainsKey(blendshapeName))
                {
                    ...
                    Debug.Log(DateTime.Now.ToString("M/d/yyyy") + " "
                + System.DateTime.Now.ToString("HH:mm:ss") + ":"
                + System.DateTime.Now.Millisecond + "," +currentBlendShapeUIs[blendshapeName].text);
